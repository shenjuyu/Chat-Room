...
public class Server {
	...
	public void startServer(){
		...
		try {
			while(started){
				Socket s=ss.accept();
				System.out.println("服务器日志:有一个客户机"+s.getInetAddress().getHostAddress()+"连接上了服务器，时间"+YcUtil.getCurrentTime());
	...
	public class ClientCon implements Runnable{
		...
		public void send(String info){
			try {
				...
				System.out.println("服务器日志：客户端"+s.getInetAddress().getHostAddress()+"已经掉线了  时间:"+YcUtil.getCurrentTime());
				e.printStackTrace();
			}
		...
		public void run() {
			while(connected){
				try {
					...
					System.out.println("服务器日志：客户端"+s.getInetAddress().getHostAddress()+"说："+info+" 时间："+YcUtil.getCurrentTime());
